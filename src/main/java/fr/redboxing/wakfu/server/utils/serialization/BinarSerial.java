...
public abstract class BinarSerial {
    ...
    public final void fromBuild(final byte[] binarBuild, final int version) {
        ...
        final ArrayList<BinarSerialPart> successParts = new ArrayList<>(parts.length);
        ...
        for (int i = 0; i < tocLength; ++i) {
            ...
            if (size <= 0) {
                WakfuServer.getInstance().getLogger().warn((Object)("Part " + index + "(offset=" + offset + ") is empty for " + this + " ! (voir log serveur)"), (Throwable)new Exception());
            }
            else if (size > 10485760) {
                WakfuServer.getInstance().getLogger().error((Object)("Part " + index + "(offset=" + offset + ") exceeds max limit (" + size + " > " + 10485760 + " bytes)"), (Throwable)new Exception());
            }
            else {
                ...
                if (index >= 0 && index < parts.length) {
                    ...
                    if (part == BinarSerialPart.EMPTY) {
                        WakfuServer.getInstance().getLogger().warn((Object)("Don't know how to unserialise part #" + index + " (EMPTY)."));
                        continue;
                    }
                    if (part == null) {
                        WakfuServer.getInstance().getLogger().error((Object)("Part " + index + " of " + this + " is null"), (Throwable)new Exception());
                        continue;
                    }
                    try {
                        part.clearError();
                    ...
                    if (partBuffer.remaining() > 0) {
                        WakfuServer.getInstance().getLogger().warn((Object)("Part " + index + " of " + this + " still have " + partBuffer.remaining() + " byte(s) left !"), (Throwable)new Exception());
                    }
                }
        ...
        for (int tocLength = buffer.get(), i = 0; i < tocLength; ++i) {
            ...
            if (tocIndex == index) {
                ...
                try {
                    ...
                    WakfuServer.getInstance().getLogger().error((Object)("Exception lev\u00e9e lors de la d\u00e9serialisation de la part :" + index), (Throwable)e);
                }
                return;
            }
        }
